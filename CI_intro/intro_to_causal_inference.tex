...
\note[itemize]{
	...
	\item{Rather than direct effect of X$\rightarrow$Y, seek out entire causal mechanisms}
}
...
\begin{frame}{Beyond Description}
	\begin{center}
		\LARGE Causal mechanisms allow us to make unbiased predictions about the effect of \textcolor{blue}{\emph{interventions}} \citep{Pearl2018} and \textcolor{blue}{\emph{counterfactual situations}}
	\end{center}
\end{frame}
\note[itemize]{
	\item{Must an intervention be something that is manipulable? Or can it represent a given state, such as a gender, height or age? Which camp do you fall into?}
...
%				\multicolumn{5}{c}{${}$} \\
		...
		\only<1>{
			\begin{wideitemize}
				\item Social media data on user behavior
